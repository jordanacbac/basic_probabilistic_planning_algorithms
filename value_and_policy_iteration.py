...
def iterative_policy_evaluation(df, states_view, proper_policy, goal, initial, row,column):
	...
	buildGrid(current_proper_policy, row, column, initial, False)
...
def value_iteration(df, states_view, goal, initial, row, column):
	...
	# Calling method responsible for the grid visualization
	buildGrid(best_action, row, column, initial, True)
...
if __name__ == '__main__':
	...
	parser.add_argument("-r", dest="row", help="grid row size", type=int)
	parser.add_argument("-c", dest="column", help="grid column size", type=int)
	...
	iterative_policy_evaluation(df,states_view,proper,goal,initial,row, column)