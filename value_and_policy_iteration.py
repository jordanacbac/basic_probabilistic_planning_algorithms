...
def iterative_policy_evaluation(df, states_view, proper_policy, goal, initial, lastState):#df iteracao de Vpi
	past_proper_policy = proper_policy.copy()
	current_proper_policy ={} #store current proper policy
	t = time.time()
	for i in df.columns:
	...
	while(True):
		...
		#print(max_residual)
			...
			#break
		#if past and current policy arent equals, reset current proper policy and store past policy with current
	...
	buildGrid(current_proper_policy, lastState, initial)
...
def value_iteration(df, states_view, goal, initial, last_state):
	...
	best_action[goal] = "-"#set goal state equals to '-', so this wont be null 
...
def buildGrid(statesANDactions, lastState, initial):
	index = lastState.split("x")[1].split("y") #[0]=row [1]=column
	grid = [ [ "*" for y in range(int(index[0])+1) ] for x in range(int(index[1])+1) ] 
	#grid = np.zeros((int(index[0]),int(index[1])))
	...
	for i in statesANDactions:
		...
		index = str(i).split("x")[1].split("y") #[0]=row [1]=column
		grid[int(index[0])][int(index[1])] = action
		#pensar em como fazer a matriz
	#initial state
	...
	for x in reversed(grid):
		print(x)
	#tratar o estado inicial e os q nao tem nada
...
if __name__ == '__main__':
	...
	'''with open("TestesGrid/FixedGoalInitialState/navigation_1.net") as file:
		content = file.readlines()'''
